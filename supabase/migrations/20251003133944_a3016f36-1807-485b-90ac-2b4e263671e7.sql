-- Fix the setup_demo_funder trigger with CORRECT net_worth_band enum value
...
BEGIN
  ...
  IF NEW.email = 'demo.funder@rift.finance' THEN
    ...
    IF demo_wallet_id IS NOT NULL AND demo_org_id IS NOT NULL THEN
      ...
      -- Create funder profile with correct enum values
      ...
      VALUES (
        ...
        'IE',
        'IE',
        '200k_1m',  -- Valid income_band value (lt_50k, 50k_200k, 200k_1m, gt_1m)
        '1m_5m',    -- Valid net_worth_band value (lt_250k, 250k_1m, 1m_5m, gt_5m)
